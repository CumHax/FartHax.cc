...
import me.cumhax.fart.util.PlaceUtil;
...
public class HoleFiller extends Module
{
	...
	private List<BlockPos> getHoles()
	{
		...
		positions.addAll(getSphere(new BlockPos(Math.floor(mc.player.posX), Math.floor(mc.player.posY), Math.floor(mc.player.posZ)), range.getIntValue(), (int) range.getIntValue(), false, true, 0).stream().filter(this::isHole).collect(Collectors.toList()));
		return positions;
	}
